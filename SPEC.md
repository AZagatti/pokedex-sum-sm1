...
Git remote (SSH): `https://github.com/AZagatti/pokedex-sum-sm1.git` (already created, empty). `git init`, set remote `origin`, commit in logical chunks, push to `main`. `paths.base` must be `/pokedex-sum-sm1`. The live URL will be `https://azagatti.github.io/pokedex-sum-sm1/`.
